...
impl Render for SettingsWindow {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let monospace_families = self.monospace_families.as_deref();
        let unicode_families = self.unicode_families.as_deref();
        let default_guifont = monospace_families.and_then(|families| families.first());
        let default_guifontwide = unicode_families.and_then(|families| families.first());

        let guifont_editor = self.font_chain_editor(
            FontChainField::GuiFont,
            &current.guifont,
            default_guifont.map(String::as_str),
            monospace_families,
            cx,
        );
        let guifontwide_editor = self.font_chain_editor(
            FontChainField::GuiFontWide,
            &current.guifontwide,
            default_guifontwide.map(String::as_str),
            unicode_families,
            cx,
        );
        ...
        root = root.on_any_mouse_down(cx.listener(|this, _, _, cx| {
            let font_list_closed = this.close_font_chain_list();
            let combo_closed = this.open_combo.take().is_some();
            if font_list_closed || combo_closed {
                cx.notify();
            }
        }));
        root = root.on_mouse_down_out(cx.listener(|this, _, _, cx| {
            this.commit_rime_path_edit(cx);
            let font_chain_closed = this.font_chain_editing.take().is_some();
            let combo_closed = this.open_combo.take().is_some();
            if font_chain_closed || combo_closed {
                cx.notify();
            }
        }));
