use super::{
    ACCENT, BACKGROUND, MAX_VISIBLE_TEXT_INPUT_CHARS, MUTED_TEXT, SURFACE, SURFACE_BRIGHT, TEXT,
};
use gpui::{
    div, fill, point, prelude::*, px, relative, rgb, App, Bounds, ClickEvent, CursorStyle,
    DispatchPhase, Element, ElementId, FocusHandle, GlobalElementId, HitboxBehavior,
    InspectorElementId, IntoElement, KeyDownEvent, LayoutId, MouseButton, MouseDownEvent,
    MouseMoveEvent, MouseUpEvent, Pixels, ShapedLine, SharedString, Stateful, Style, TextRun,
    Window,
};
...
pub(crate) enum TextInputMouseEvent {
    Down { index: usize, shift: bool },
    Drag { index: usize },
    Up,
}

type TextInputClickHandler = Box<dyn Fn(&ClickEvent, &mut Window, &mut App)>;
type TextInputKeyHandler = Box<dyn Fn(&KeyDownEvent, &mut Window, &mut App)>;
type TextInputMouseHandler = Box<dyn Fn(TextInputMouseEvent, &mut Window, &mut App)>;
type SharedTextInputMouseHandler = Rc<dyn Fn(TextInputMouseEvent, &mut Window, &mut App)>;
...
pub(crate) struct TextInputConfig {
    ...
    on_click: TextInputClickHandler,
    on_key_down: TextInputKeyHandler,
    on_mouse: TextInputMouseHandler,
}
...
impl TextInputConfig {
    ...
    pub(crate) fn on_mouse(
        mut self,
        handler: impl Fn(TextInputMouseEvent, &mut Window, &mut App) + 'static,
    ) -> Self {
...
struct TextInputText {
    ...
    focus_handle: FocusHandle,
    on_mouse: Option<TextInputMouseHandler>,
}
...
impl Element for TextInputText {
    ...
    fn prepaint(
        ...
        TextInputPrepaintState { line, hitbox }
    }
    ...
    fn paint(
        ...
        if let Some(on_mouse) = self.on_mouse.take() {
            let on_mouse: SharedTextInputMouseHandler = Rc::from(on_mouse);
            ...
            window.on_mouse_event(move |event: &MouseMoveEvent, phase, window, cx| {
                    ...
                    && move_hitbox.is_hovered(window)
                {
                    let index =
                        text_input_index_for_position(event.position, &display, &layout, bounds);
                    move_on_mouse(TextInputMouseEvent::Drag { index }, window, cx);
                }
            });

            window.on_mouse_event(move |event: &MouseUpEvent, phase, window, cx| {
                if phase == DispatchPhase::Bubble && event.button == MouseButton::Left {
                    on_mouse(TextInputMouseEvent::Up, window, cx);
                }
            });
        }
    }
...
pub(crate) fn text_input(config: TextInputConfig) -> Stateful<gpui::Div> {
    let TextInputConfig {
        id,
        state,
        placeholder,
