...
impl NvimGpui {
    ...
    pub(crate) fn start_open_urls_task(
        ...
        self.open_urls_task = Some(cx.spawn(async move |weak, cx| {
    ...
    pub(crate) fn on_file_drag_move(
        &mut self,
        _event: &gpui::DragMoveEvent<gpui::ExternalPaths>,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        if self.nvim.as_ref().is_some_and(NvimProcess::is_remote) && self.file_drop_notice.is_none()
        {
    ...
    fn show_remote_file_drop_notice(&mut self, cx: &mut Context<Self>) {
        self.file_drop_notice_generation = self.file_drop_notice_generation.wrapping_add(1);
        let generation = self.file_drop_notice_generation;
        self.file_drop_notice = Some(REMOTE_FILE_DROP_NOTICE.to_owned());
        self.file_drop_notice_task = Some(cx.spawn(async move |weak, cx| {
            gpui::Timer::after(FILE_DROP_NOTICE_DURATION).await;
            let _ = weak.update(cx, |this, cx| {
                if this.file_drop_notice_generation == generation {
                    this.file_drop_notice = None;
                    this.file_drop_notice_task = None;
                    cx.notify();
                }
            });
        }));
